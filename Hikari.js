...
const listprod = require("./FuncBot/product");
...
let _product = JSON.parse(fs.readFileSync('./database/product.json'));
let _leveling = JSON.parse(fs.readFileSync('./database/group/leveling.json'))
let _level = JSON.parse(fs.readFileSync('./database/user/level.json'))
let productnya =JSON.parse(fs.readFileSync('./database/product.json'));
...
Hikari.sendMessage(from, {"contentText": `${menu}`,"footerText": `${setting.botName} By @${setting.ownerName}`,"buttons": [{buttonId: `${prefix}command`, buttonText: {displayText: '⎙Command'}, type: 1},{buttonId: `${prefix}Rules`, buttonText: {displayText: 'S&K Bot'}, type: 1},{buttonId: `${prefix}owner`, buttonText: {displayText: 'Owner'}, type: 1}],
...
//CASE PRODUK BY MANIK NO COPAS
case 'addproduct':{
              if (args.length == 0) return reply(`Example: ${prefix + command} NamaProduk|List`)
              const textp = body.slice(12)
              const namaprod = textp.split("|")[0]
              const listrod = textp.split("|")[1]
              listprod.addProdUser(namaprod, time, listrod, _product)
              const aluty = `*Anda berhasil Menambah Produk!*\n\n➸ *Nama Produk*: ${namaprod}\n➸ *List Produk*: ${listrod}`
              reply(aluty)
break
}
case 'cekproduct':{
const textp = q
const getId = listprod.getProdId(textp, _product)
const getReason = listprod.getProdReason(getId, _product)
const rpg = `*${getReason}* `
reply(rpg)
break
}
...
const hhj = ['Kuat', 'Lemah','Kuat', 'Lemah','Kuat', 'Lemah','Kuat', 'Lemah']
